...
@click.option("--ref-audio", "-r", required=True, help="Path or URL to reference audio")
@click.option(
    "--ref-text",
    "-s",
    help="Transcription of reference audio (optional - will auto-transcribe if not provided)",
)
@click.option("--output", "-o", help="Output file path")
@click.option("--language", "-l", default="English", help="Target language")
@click.option(
    "--device",
    "-d",
    default=None,
    help="Device to use (cuda:0, mps, cpu, or auto-detect if not specified)",
)
@click.option(
    "--force-transcribe",
    "-f",
...
def clone(text, ref_audio, ref_text, output, language, device, force_transcribe):
    ...
    click.echo(f"Initializing voice cloner on {device or 'auto-detected device'}...")
    ...
    try:
        ...
        transcript = cloner.get_or_create_transcript(
            ref_audio=ref_audio, ref_text=ref_text, force_transcribe=force_transcribe
        )
...
@click.option(
    ...
    "-d",
    default=None,
    help="Device to use for transcription (auto-detect if not specified)",
)
@click.option(
    "--ref-text",
    "-s",
...
def clone_from_file(
    text, ref_audio, ref_text, output_dir, language, device, force_transcribe
):
...
@main.command()
@click.option("--audio", "-a", required=True, help="Path to audio file to transcribe")
@click.option(
